...
def get_floor_by_id(session: Session, floor_id: int) -> Floor | None:
    statement = select(Floor).where(Floor.id == floor_id).options(selectinload(Floor.table_groups.and_(TableGroup.is_active == True)))
    return session.exec(statement).first()

def get_all_floors(session: Session) -> list[Floor]:
    statement = select(Floor).options(selectinload(Floor.table_groups.and_(TableGroup.is_active == True)))
