...
def get_sales_history(
    ...
    if dish_name or category_id is not None:
        query = (
            query.join(Order.detail)
            .join(OrderDetail.price)
            .join(DishPrice.dish)
        )
        if dish_name:
...
def get_dashboard_stats(
    ...
    payment_query = (
        ...
        .options(
            ...
            .selectinload(Order.tablegroup)
            .selectinload(TableGroup.floor)
        )
    )
    if start_date:
    ...
    detail_query = (
        select(
            Dish.name, func.sum(OrderDetail.quantity).label("total_quantity")
        )
        .join(DishPrice, OrderDetail.price_id == DishPrice.id)
        .join(Dish, DishPrice.dish_id == Dish.id)
    ...
    detail_query = (
        detail_query.group_by(Dish.name)
    ...
    top_dishes = [
        TopDish(name=row[0], quantity=row[1]) for row in top_dishes_raw
    ]
    ...
    sales_per_day = [
        SalesPerDay(date=k, revenue=v) for k, v in sorted(daily_stats.items())
    ]
    ...
    discount_recommendations = ml_service.generate_discount_recommendations(
        session
    )
