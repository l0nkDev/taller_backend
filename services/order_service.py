...
def get_order_by_tablegroup(session: Session, tablegroup_id: int) -> Order | None:
    return session.exec(select(Order).where(Order.tablegroup_id == tablegroup_id, Order.was_cancelled == False, Order.was_paid == False)).first()
...
def pay_order(session: Session, order_id: int, payment_data: PaymentCreate) -> Payment:
    db_order = session.get(Order, order_id)
    ...
    payment = Payment(method=payment_data.method, total=total, order_id=db_order.id)
    session.add(payment)
    session.commit()
