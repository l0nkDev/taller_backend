...
def seed_data():
    with Session(engine) as session:
        ...
        if not users:
            print(
                "No users found. Please create some users first via the app."
            )
            return
        users[0]
        ...
        for day_offset in range(90):
            ...
            for _ in range(num_orders):
                ...
                payment = Payment(
                    ...
                    total=total_amount,
                    created_at=order_time
                    + timedelta(minutes=random.randint(45, 90)),
                )
        ...
        session.commit()
        print(
            "Seeding complete! Added 3 months of historical data with high volume."
        )
