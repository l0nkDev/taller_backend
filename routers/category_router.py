...
def create_category_endpoint(payload: CategoryCreate, session: Session = Depends(get_session),
...
def update_category_endpoint(category_id: int, payload: CategoryCreate, session: Session = Depends(get_session),
                             current_user: User = Depends(require_admin)):
