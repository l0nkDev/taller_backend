...
def test_update_dish_price_change(session: Session):
    # Arrange
    initial_data = DishCreate(name="Majadito", description="", available=True, price=25.0, category_id=1)
    ...
    old_price = next(p for p in updated_dish.prices if p.id == original_price_id)
...
def test_update_dish_same_price_does_not_create_new_record(session: Session):
    # Arrange
    initial_data = DishCreate(name="Locro", description="", available=True, price=20.0, category_id=1)
    db_dish = create_new_dish(session, initial_data)
